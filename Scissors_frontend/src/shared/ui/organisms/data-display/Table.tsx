...
interface TableAction<T extends TableRow> {
  label: string;
  className: string;
  onClick: (row: T) => void;
  isDynamic?: boolean;
...
const getCellValue = <T extends TableRow>(row: T, accessor: string): unknown => {
  return (row as Record<string, unknown>)[accessor];
};

const Table = <T extends TableRow>({ columns, data, actions }: TableProps<T>) => {
          ...
          {data.map((row, index) => (
            <tr key={row._id || index} className="hover:bg-gray-50">
              {columns.map((col) => (
                ...
                >
                  {typeof getCellValue(row, col.accessor) === "boolean" ? (
                    getCellValue(row, col.accessor) ? (
                      <span className="text-green-600">Active</span>
                    ) : (
                      <span className="text-red-600">Blocked</span>
                    )
                  ) : typeof getCellValue(row, col.accessor) === "object" && getCellValue(row, col.accessor) !== null ? (
                    <div className="space-y-1">
                      {Object.entries(getCellValue(row, col.accessor) as object).map(([key, value]) => (
