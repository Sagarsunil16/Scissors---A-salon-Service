...
import { Chat, IMessage } from "@/types/Imessage";
...
export const verifyOTP = async (data: { email: string; otp: string; purpose?: "password-reset" }) => {
...
export const getAllSalons = async (params: URLSearchParams) => {
  return await PublicAPI.get("/salons", { params });
};
export const getSalonDetails = async (id: string) => {
  return await PublicAPI.get("/salon-details", {
    params: { id: id },
  });
};
...
export const fetchServiceStylist = async (data: { salonId: string; serviceIds: string[],date:string }) => {
  return PublicAPI.get(`/salons/${data.salonId}/stylist`, {
    params: {
...
export const getSalonReviews = async (salonId: string) => {
  return await PublicAPI.get(`/salons/${salonId}/reviews`);
};
...
export const getCheckoutSessionStatus = async (sessionId: string) => {
  return await API.get(`/checkout-session/${sessionId}/status`);
};
...
export const getSalons = async (params: {
  ...
  const response = await PublicAPI.get("/salons", { params });
  return response;
};
